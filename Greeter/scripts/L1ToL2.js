...
const main = async () => {
  ...
  const [_submissionPriceWei, nextUpdateTimestamp] =
    await bridge.l2Bridge.getTxnSubmissionPrice(newGreetingBytesLength) // the user is charged the base submission fee to cover the storage costs of keeping their ticket’s calldata in the retry buffer. (current base submission fee is queryable via ArbRetryableTx.getSubmissionPrice)
  console.log(
   ...
   */
  //a single txn can trigger many messages,但是在这个案例中我们仅仅打包了一个tx
  const ourMessagesSequenceNum = inboxSeqNums[0]

  // 获取对应的retryableTxnHash
  ...
  //  在l2上retryableTxnHash对应的交易执行完成后返回的receipt
  /*
  隐含过程
    operator  --> redemption retryableTicket --> execute tx --> generate receipt
  */
